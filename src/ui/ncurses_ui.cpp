...
std::string Selection::toString() const
{
  return "Selection(" + std::to_string(start_buf_line) + "," +
         std::to_string(start_col) + " -> " + std::to_string(end_buf_line) +
...
Window::Window(int height, int width, int start_y, int start_x)
    : win_(nullptr), height_(height), width_(width), start_y_(start_y),
      start_x_(start_x)
{
}
...
ContentWindow::ContentWindow(int height, int width, int start_y, int start_x,
                             size_t max_lines)
    : Window(height, width, start_y, start_x), max_lines_(max_lines),
      scroll_offset_(0)
{
}
...
int ContentWindow::window_row_to_buffer_line(int win_row, int *out_col_offset)
{
  ...
  for (size_t buf_idx = start_line; buf_idx < total_lines; buf_idx++)
  {
    ...
    for (int line_row = start_row_in_line; line_row < line_rows; line_row++)
    {
      ...
      {
        if (out_col_offset)
          *out_col_offset = line_row * width_;
        return buf_idx;
      }
      screen_row++;
    }
...
std::string ContentWindow::extract_selected_text()
{
  ...
  for (int buf_line = l1; buf_line <= l2 && buf_line < (int)buffer_.size();
       buf_line++)
  {
    if (buf_line < 0)
      continue;
...
void ContentWindow::render_buffer()
{
  ...
  // 绘制内容行（支持软换行）
  int row = 0;
  for (size_t buf_idx = start_line;
       buf_idx < total_lines && row < content_height; buf_idx++)
  {
    const std::string &line = buffer_[buf_idx];
    int line_rows = calc_wrapped_rows(buf_idx);
    ...
    // 绘制该逻辑行的各个软换行片段
    for (int line_row = start_row_in_line;
         line_row < line_rows && row < content_height; line_row++)
    {
  ...
  if (in_prompt_mode_)
  {
    ...
    if (password_mode_)
    {
      ...
      mvwprintw(win_, height_ - 1, 0, "[%s] %s", prompt_display.c_str(),
                stars.c_str());
    }
    else
    {
...
void ContentWindow::handle_mouse_event(MEVENT &event)
{
  ...
  bool is_drag_event =
      mouse_button1_down_ ||
      (mouse_button1_down_ && ((event.bstate & BUTTON1_CLICKED) ||
...
void ContentWindow::handle_scroll_with_selection(int direction, int mouse_y,
                                                 int mouse_x)
{
...
std::string ContentWindow::get_current_input() const { return current_input_; }
...
std::string ContentWindow::get_selected_text() const { return selected_text_; }

bool ContentWindow::has_selection() const { return selection_.valid; }
...
void ContentWindow::set_on_command(
    std::function<void(const std::string &)> callback)
{
  on_command_ = callback;
}

void ContentWindow::set_on_selection(
    std::function<void(const std::string &)> callback)
{
...
{
  std::lock_guard<std::recursive_mutex> lock(mutex_);
  status_lines_.erase(std::remove_if(status_lines_.begin(), status_lines_.end(),
                                     [row](const auto &p)
                                     { return p.first == row; }),
                      status_lines_.end());
}
...
void NCursesUI::on_resize_signal(int sig) { resize_pending_.store(true); }

bool NCursesUI::check_resize() { return resize_pending_.load(); }
...
{
  if (content_win_)
    content_win_->print(text);
}

void NCursesUI::add_line(const std::string &line)
{
  if (content_win_)
    content_win_->add_line(line);
}
...
void NCursesUI::set_on_command(
    std::function<void(const std::string &)> callback)
{
  if (content_win_)
    content_win_->set_on_command(callback);
}

void NCursesUI::set_on_selection(
    std::function<void(const std::string &)> callback)
{
  if (content_win_)
    content_win_->set_on_selection(callback);
}
...
bool copy_to_clipboard(const std::string &text)
{
  ...
  std::string ps_cmd = "powershell.exe -command \"Set-Clipboard -Value '" +
                       text + "'\" 2>/dev/null";
...
int ContentWindow::prompt_int(const std::string &prompt_text, int default_value,
                              int min_val, int max_val)
{
  std::string default_str =
      (default_value != 0) ? std::to_string(default_value) : "";
  ...
  while (true)
  {
    ...
    try
    {
      ...
      {
        add_line("Out of range! Must be between " + std::to_string(min_val) +
...
double ContentWindow::prompt_float(const std::string &prompt_text,
                                   double default_value, double min_val,
                                   double max_val)
{
  std::string default_str =
      (default_value != 0.0) ? std::to_string(default_value) : "";
...
bool ContentWindow::prompt_yn(const std::string &prompt_text, bool default_yes)
{
  ...
  {
    std::string input =
        prompt(prompt_text + " (" + default_str + ")", default_yes ? "y" : "n");
                                 ...
                                 int default_idx)
{
  ...
  {
    std::string input =
        prompt("Enter choice (1-" + std::to_string(options.size()) + "):",
               default_str);
...
std::string ContentWindow::prompt_validate(
