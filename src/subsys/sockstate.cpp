...
int SockState::do_connect(int fd, const struct sockaddr *addr, socklen_t addrlen) {
...
ssize_t SockState::do_sendto(int fd, const void *buf, size_t len, int flags,
    ...
    LOG_TRACE("sendto(fd=%d, len=%zu) to %s", fd, len, format_sockaddr(dest).c_str());
...
ssize_t SockState::do_recvfrom(int fd, void *buf, size_t len, int flags,
    ...
    UdpDatagram &dg = it->second.front();
    ...
    if (src_addr && addrlen) {
        ...
        socklen_t addr_len = std::min(guest_addrlen, (socklen_t)sizeof(struct sockaddr_in));
    ...
    LOG_TRACE("recvfrom(fd=%d) -> len=%zu from %s", fd, copy_len, 
              format_sockaddr(dg.from).c_str());
...
int SockState::do_getsockname(int fd, struct sockaddr *addr, socklen_t *addrlen) {
    ...
    if (addr && addrlen) {
        ...
        socklen_t len = std::min(guest_addrlen, (socklen_t)sizeof(struct sockaddr_in));
...
bool SockState::is_valid_socket(int fd) const {
    return sockets_.count(fd) > 0;
}
...
size_t SockState::get_udp_buffer_size(int fd) const {
    auto it = udp_recv_buffers_.find(fd);
    if (it != udp_recv_buffers_.end()) return it->second.size();
    return 0;
}
...
void Socket::serialize(Archive &ar) {
    ...
    ar(fd, domain, type, protocol, bound, 
       local_family, local_port, local_addr_val,
       connected, listening, 
       peer_family, peer_port, peer_addr_val,
...
void TcpConnection::serialize(Archive &ar) {
    ...
    ar(local_fd, peer_fd, peer_pid, 
       l_family, l_port, l_addr,
       p_family, p_port, p_addr,
...
template void Socket::serialize<cereal::BinaryInputArchive>(cereal::BinaryInputArchive &);
template void Socket::serialize<cereal::BinaryOutputArchive>(cereal::BinaryOutputArchive &);
template void UdpDatagram::serialize<cereal::BinaryInputArchive>(cereal::BinaryInputArchive &);
template void UdpDatagram::serialize<cereal::BinaryOutputArchive>(cereal::BinaryOutputArchive &);
template void TcpConnection::serialize<cereal::BinaryInputArchive>(cereal::BinaryInputArchive &);
template void TcpConnection::serialize<cereal::BinaryOutputArchive>(cereal::BinaryOutputArchive &);
template void SockState::serialize<cereal::BinaryInputArchive>(cereal::BinaryInputArchive &);
template void SockState::serialize<cereal::BinaryOutputArchive>(cereal::BinaryOutputArchive &);
...
static void addr_str_to_sockaddr(const std::string &addr_str, struct sockaddr_in *sin) {
...
ssize_t emu_recvfrom(int sockfd, void *buf, size_t len, int flags,
                     struct sockaddr *src_addr, socklen_t *addrlen) {
    int cur = ptmc_state.cursor;
    return ptmc_state.sock_states[cur].do_recvfrom(sockfd, buf, len, flags, src_addr, addrlen);
}
...
ssize_t emu_sendto(int sockfd, const void *buf, size_t len, int flags,
    ...
    LOG_TRACE("emu_sendto: delivered %zu bytes to node %d fd %d", len, node, recvfd);
