...
void FutexState::add_waiter(const FutexWaiter &waiter)
{
  ...
  LOG_DEBUG("Futex: TID %d added to wait queue for addr %p", waiter.tid,
            (void *)waiter.uaddr);
}
...
bool FutexState::remove_waiter(pid_t tid)
{
  ...
  for (auto qit = queue.begin(); qit != queue.end(); ++qit)
  {
    if (qit->tid == tid)
    {
      queue.erase(qit);
      waiter_map_[tid] = 0;
      LOG_DEBUG("Futex: TID %d removed from wait queue for addr %p", tid,
                (void *)uaddr);
      return true;
    }
  }
  ...
  switch (op)
  {
    case FUTEX_WAIT:
    {
      ...
      if (stackval)
      {
        ...
        {
          LOG_DEBUG("FUTEX_WAIT: value mismatch (expected %d, got %lu)", val,
                    *stackval);
          return -EAGAIN;
        }
      }
      ...
      LOG_DEBUG("FUTEX_WAIT: TID %d waiting on %p (val=%d)", tid, (void *)uaddr,
                val);
      return 0;
    }
    ...
    case FUTEX_WAKE_BITSET:
    {
      ...
      LOG_DEBUG("FUTEX_WAKE_BITSET: woke %d threads from %p (bitset=%u)", woken,
                (void *)uaddr, bitset);
      return woken;
    }
    ...
    case FUTEX_REQUEUE:
    {
      ...
      LOG_DEBUG("FUTEX_REQUEUE: woken=%d, requeued=%d from %p to %p", woken,
                requeued, (void *)uaddr, (void *)uaddr2);
      return woken;
    }
    ...
    case FUTEX_CMP_REQUEUE:
    {
      ...
      return handle_futex(tid, uaddr,
                          FUTEX_REQUEUE | (futex_op & ~FUTEX_CMD_MASK), val,
                          timeout_addr, uaddr2, val3, stackval);
    }
...
bool FutexState::wake_thread(pid_t tid) { return remove_waiter(tid); }
...
int FutexState::futex_requeue(uint64_t uaddr1, uint64_t uaddr2, int nr_wake,
                              int nr_requeue)
{
...
int FutexState::wake_waiters_in_range(uint64_t start, uint64_t end)
{
  ...
  {
    LOG_INFO(
        "Futex: woke %d waiters due to memory range [%p, %p) being discarded",
