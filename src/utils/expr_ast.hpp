...
class UnaryOpNode : public ExprNode {
    ...
    ExprPtr operand_;
public:
    UnaryOpNode(UnaryOp op, ExprNode* opnd)
        : op_(op), operand_(opnd) {}
    EvalResult eval(int pid, bool& success) const override;
    std::string to_string() const override;
};

/* Prefix ++ -- */
enum class PrefixOp { PRE_INC, PRE_DEC };

class PrefixOpNode : public ExprNode {
    PrefixOp op_;
    ExprPtr operand_;
public:
    PrefixOpNode(PrefixOp op, ExprNode* opnd)
        : op_(op), operand_(opnd) {}
    EvalResult eval(int pid, bool& success) const override;
    std::string to_string() const override;
};
...
/* Postfix ++ -- */
enum class PostfixOp { POST_INC, POST_DEC };

class PostfixOpNode : public ExprNode {
    PostfixOp op_;
    ExprPtr operand_;
public:
    PostfixOpNode(PostfixOp op, ExprNode* opnd)
        : op_(op), operand_(opnd) {}
    EvalResult eval(int pid, bool& success) const override;
    std::string to_string() const override;
};
...
class ArrayIndexNode : public ExprNode {
    ...
    ExprPtr index_;
public:
    ArrayIndexNode(ExprNode* arr, ExprNode* idx)
