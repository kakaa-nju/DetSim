...
bool FileLock::acquire(bool blocking, int timeout_seconds)
{
  ...
  int retries = 0;
  int max_retries =
      blocking ? (timeout_seconds > 0 ? timeout_seconds * 10 : INT_MAX) : 1;
  ...
  while (retries < max_retries)
  {
    ...
    if (!is_process_alive(owner_pid))
    {
      ...
      char temp_lock[256];
      snprintf(temp_lock, sizeof(temp_lock), "%s.tmp.%d", lock_path.c_str(),
               my_pid);
...
bool FileLockGuard::is_locked() const { return locked_; }
