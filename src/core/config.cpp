/*
 * config.cpp - Configuration parsing implementation using modern C++ (nlohmann/json)
 */
...
static std::string resolve_path(const std::string &path, const fs::path &cfg_dir)
{
...
static void parse_tracees(const json &cfg, const fs::path &cfg_dir)
{
  ...
  Assert(nodes == NP,
         "Build doesn't support %d processes, please compile with 'NP=%d' and rebuild",
         nodes, nodes);
...
static void parse_fsmap(const json &cfg, const fs::path &cfg_dir)
{
  ...
  for (const auto &item : fsmap) {
    if (item.contains("Host") && item.contains("Target")) {
      std::string host_path = resolve_path(item["Host"].get<std::string>(), cfg_dir);
  ...
  if (access(obj_name.c_str(), R_OK | X_OK) != 0) {
    ...
    if (pid == 0) {
      ...
      execlp("g++", "g++", "-O2", arg_d, "-fpic", "-shared",
             ...
             src_path.c_str(), "-o", obj_name.c_str(), nullptr);
      perror("exec");
      _exit(1);
    } else {
...
{
  if (!cfg.contains("UserCheck")) return;
...
{
  if (!cfg.contains("ChooseFunc")) return;
  ...
  for (int nr = 0; nr < 450; nr++) {
    if (syscalls[nr] == nullptr) continue;
...
{
  if (!cfg.contains("StateStore")) return;
  ...
  if (ss.contains("malloc_trim_threshold_mb")) {
    ss_config.malloc_trim_threshold = ss["malloc_trim_threshold_mb"].get<size_t>() * 1024 * 1024;
  }
...
void parse_args(int argc, char *argv[])
{
  ...
  int o;
  while ((o = getopt_long(argc, argv, "-ahl:c:", table, nullptr)) != -1) {
    switch (o) {
      case 'a':
        auto_mode = 1;
        break;
