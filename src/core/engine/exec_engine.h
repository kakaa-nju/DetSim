...
struct ExecResult
{
  ...
  ExecResult(ExecStatus s = ExecStatus::SUCCESS, int ckpt = 0,
             const char *msg = "")
...
class ExecutionEngine
{
  ...
  void on_thread_created(pid_t parent_pid, pid_t new_tid, uint64_t clone_flags,
                         uint64_t stack_addr);

  // Handle thread exit
  bool on_thread_exit(pid_t pid, pid_t wait_result, int wstatus,
                      syscall_info &si);
