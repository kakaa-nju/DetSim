...
- **单机模拟分布式系统**: 支持单机模拟多节点分布式系统的执行
- **Stateful 状态管理**: 保存程序执行状态（与 Stateless 保存执行路径相对），支持快照的保存、加载和比较
- **执行调度控制**: 调度交错粒度可调节（指令级、系统调用级）
- **自动探索**: 广度优先搜索所有可能的执行路径
- **交互式调试**: 支持断点、单步执行、状态回溯、表达式求值
- **表达式求值**: 类似 GDB 的表达式求值，支持变量、数组、指针、结构体、类型转换
- **状态比较**: 对比不同执行路径的系统状态差异
- **调用栈分析**: 支持 backtrace、栈帧切换、局部变量查看
...
- [x] 实现易用的调试模式
  - [x] 变量分析，类型推导
  - [x] 表达式运算（支持结构体、指针、类型转换）
  - [x] 更方便的操控方式（进程切换、栈帧切换）
  - [x] 友好的信息展示（GDB 风格输出）
- [x] 状态比较功能：支持对比两个全局状态的差异（寄存器、内存、文件系统）
...
| `x FORMAT ADDR` | 打印内存内容（支持格式 bhwg/hdou） |
| `ls` | 列出当前进程文件系统中的文件 |
| `stat PATTERN` | 显示匹配模式的文件状态 |
| `hexdump PATH` | 十六进制显示文件内容 |
| `batch FILE` | 从文件批量读取命令 |
| `q` | 退出 |

**diff 命令**: 比较两个系统状态的快照，显示寄存器、内存、文件系统等差异。
- `-b, --brief`: 仅显示概览
- `-m, --memory-only`: 仅比较内存
- `--max-diff=N`: 限制显示的差异数量

**表达式求值**: 支持完整的 C 表达式语法，详见 [表达式求值文档](expr.md)。

示例：
```
(pmc) p g_basic.x                    # 基本变量
(pmc) p g_arr[2]                     # 数组索引
(pmc) p *g_ptr                       # 指针解引用
(pmc) p g_struct->field              # 结构体访问
(pmc) p ((Type*)g_void_ptr)->member  # 类型转换
(pmc) p tracee1(g_remote_var)        # 进程限定
```
