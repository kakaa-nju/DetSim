    ...
    ...) noexcept
{
  ...
  if (should_log(level)) {
    ...
    std::string levelstr;
    switch (level) {
      case 0: levelstr = FG_BLACK   "trace" RESET; break;
      case 1: levelstr = FG_BLUE    "debug" RESET; break;
      case 2: levelstr = FG_GREEN   "info"  RESET; break;
      case 3: levelstr = FG_YELLOW  "warn"  RESET; break;
      case 4: levelstr = FG_RED     "error" RESET; break;
      case 5: levelstr = FG_MAGENTA "crit"  RESET; break;
      default: assert(0);
    }
    fprintf(stdout, "[%s.%03ld][%s][%s:%d] ", 
        buf, tv.tv_usec / 1000,
        levelstr.c_str(),
        file, line 
        );
