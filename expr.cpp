...
enum {
  TK_NOTYPE = 254, TK_OR, TK_AND, TK_NOT, TK_EQ = 258, TK_PLUS = 260,
  TK_MINUS, TK_MUL = 264, TK_DIV, TK_NEG, TK_DEREF = 270, TK_MEMBER, TK_SYM,
  TK_DEC = 280, TK_VAR, TK_LP, TK_RP,
};
  ...
  while (e[position] != '\0') 
  {
    ...
    for (i = 0; i < NR_REGEX; i++) 
    {
      if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 &&
        ...
        LOG_TRACE(
            "match rules[%d] = \"%s\" at position %d with len %d:"
...
op eval(int p, int q, bool *success) {
  ...
  else 
  {
    ...
    if (tk_type == TK_SYM) 
    {
      ...
      ret = *(long *)mem;
      free(mem);
      LOG_DEBUG("get %s:%s = %ld at addr 0x%lx", tokens[p].str, tokens[q].str, ret, addr);
